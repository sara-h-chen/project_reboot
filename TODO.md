...
- [x] Stress servers as AI bots wander the map and frequently switch zones.

- [x] Stress with failure of component(s).

- [ ] Run all experiments with larger bot numbers.
...
- [x] Porting single-server applications to multi-server architectures. Difficulties faced due to code dependencies.
...
- [x] Large packets sent over the network may increase latency, but required when players are switching from one server to another.

- [x] Overlapping region where two servers share information about the player's behavior, so they both keep a copy of the player.
...
- [x] Large lag during transfer if not loaded in advance
...
- [x] Choosing the right threshold is non-trivial.

- [x] Redis creates a new connection for every Redis client.
...
- [x] How do you decide what the right threshold is?
...
- [x] Do not respond to temporary spikes, because otherwise may generate more latency/workload than it saves.
...
- [x] Moving boundaries gives you an aggregate effect where too many players might end up getting sent to adjacent servers.
...
- [x] No arbitrary thresholds are set, because you want best-effort on all servers, and they should adapt.
...
- [x] Edge servers with no overlapping regions are more likely to be overloaded due to the fact that they can only shed to _one_ neighbouring server.
